...
<Info>
  The assistant requires a tested and enabled AI provider. See [Using Artificial
  Intelligence](/guides/using-ai) for setup and [Using AI in the
  builder](/guides/using-ai-in-the-builder) for the builder workflow.
</Info>
...
<Warning>
  Copying the URL does not make the resume public by itself. To allow visitors to open the link,
  enable public access in the **Sharing** section of the right sidebar.
</Warning>
...
<Tip>
  Use the dock when you already know which file you need. Use the right sidebar's **Export** section
  when you want the format descriptions next to each button.
</Tip>
