...
describe("useControlledState", () => {
  ...
  it("updates internal state when value prop changes (controlled)", () => {
    const { result, rerender } = renderHook(
      ({ value }: { value: number }) => useControlledState({ value }),
      {
  ...
  it("calls onChange when state is updated", () => {
    ...
    const { result } = renderHook(() =>
      useControlledState<string>({ defaultValue: "a", onChange }),
    );
  ...
  it("forwards extra args to onChange", () => {
    const onChange = vi.fn();
    const { result } = renderHook(() =>
      useControlledState<string, [number, boolean]>({ defaultValue: "a", onChange }),
    );
