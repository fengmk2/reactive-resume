...
function ProviderRow({ provider }: ProviderRowProps) {
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: orpc.aiProviders.list.queryKey() });
  const { mutate: testProvider, isPending: isTesting } = useMutation(
    orpc.aiProviders.test.mutationOptions(),
  );
  const { mutate: updateProvider, isPending: isUpdating } = useMutation(
    orpc.aiProviders.update.mutationOptions(),
  );
  const { mutate: deleteProvider, isPending: isDeleting } = useMutation(
    orpc.aiProviders.delete.mutationOptions(),
  );
            ...
            onCheckedChange={(enabled) =>
              updateProvider(
                { id: provider.id, enabled },
                {
                  ...
                  onError: (error) =>
                    toast.error(
                      getOrpcErrorMessage(error, { fallback: t`Failed to update provider.` }),
                    ),
                },
              )
            }
                ...
                onError: (error) => {
                  toast.error(
                    getOrpcErrorMessage(error, {
                      fallback: t`Could not verify provider connection.`,
                    }),
                  );
                  void invalidate();
                },
          ...
          onClick={() =>
            deleteProvider(
              { id: provider.id },
              {
                ...
                onError: (error) =>
                  toast.error(
                    getOrpcErrorMessage(error, { fallback: t`Failed to delete provider.` }),
                  ),
              },
            )
          }
...
function CreateProviderForm() {
  ...
  const { mutate: createProvider, isPending } = useMutation(
    orpc.aiProviders.create.mutationOptions(),
  );
              ...
              },
              {
                onSuccess: () => {
                  ...
                  void queryClient.invalidateQueries({
                    queryKey: orpc.aiProviders.list.queryKey(),
                  });
                },
                onError: (error) =>
                  toast.error(
...
export function AISettingsSection() {
  ...
  const hasUsableProvider = providers?.some(
    (provider) => provider.enabled && provider.testStatus === "success",
  );
    ...
    <section className="grid gap-6">
      ...
      {error ? (
        ...
        >
          {isConfigError ? (
            <Trans>
              AI provider management is unavailable until REDIS_URL and ENCRYPTION_SECRET are
              configured.
            </Trans>
          ) : (
