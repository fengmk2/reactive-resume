...
export function ImportResumeDialog(_: DialogProps<"resume.import">) {
  ...
  const hasAIProvider =
    aiProviders?.some((provider) => provider.enabled && provider.testStatus === "success") ?? false;
      ...
      try {
        ...
        if (value.type === "pdf") {
          if (isLoadingAiProviders)
            throw new Error(t`Loading AI providers. Please try again in a moment.`);
          if (!hasAIProvider)
            throw new Error(
              t`This feature requires a tested AI provider. Please add one in the settings.`,
            );
  ...
  return (
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          <Trans>
            Continue where you left off by importing an existing resume you created using Reactive
            Resume or any another resume builder. Supported formats include PDF, Microsoft Word, as
            well as JSON files from Reactive Resume.
          </Trans>
        </DialogDescription>
      </DialogHeader>
