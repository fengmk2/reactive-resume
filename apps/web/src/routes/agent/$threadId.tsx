...
import {
  ...
  ResizablePanel,
  ResizableSeparator,
} from "@reactive-resume/ui/components/resizable";
...
function MessagePart({
  ...
  if (part.type === "tool-ask_user_question") {
    const input =
      "input" in part && typeof part.input === "object" && part.input
    ...
    const choices = Array.isArray(input.choices)
      ...
      : [];
    const question =
      typeof input.question === "string" ? input.question : t`The agent needs your input.`;
        ...
        <div className="flex flex-wrap gap-2">
          {choices.map((choice) => (
            <Button
              ...
              size="sm"
              variant="outline"
              onClick={() => onAnswer(part.toolCallId, choice)}
            >
              {choice}
            </Button>
          ))}
...
function AgentChat({
  ...
  const transport = useMemo(
    () => ({
      ...
      async reconnectToStream() {
        return parseAgentSseStream(
          eventIteratorToUnproxiedDataStream(
            await streamClient.agent.messages.resume({ threadId }),
          ),
        );
      },
    }),
  ...
  useEffect(() => {
    ...
    for (const message of messages) {
      for (const part of message.parts) {
        ...
        const output =
          typeof part.output === "object" ? (part.output as Record<string, unknown>) : null;
        const actionId = typeof output?.actionId === "string" ? output.actionId : null;
        const toolCallId =
  ...
  const send = () => {
    ...
    if ((!text && pendingAttachments.length === 0) || isReadOnly || isStreaming || isUploading)
      return;
  ...
  return (
    <div className="border-amber-300 border-b bg-amber-50 px-4 py-2 text-amber-950 text-sm dark:bg-amber-950/20 dark:text-amber-200">
      {readOnlyReason === "archived" ? (
        ...
        <Trans>
          This thread is read-only because the working resume or AI provider is unavailable.
        </Trans>
      )}
    </div>
  );
      ...
      <div className="mx-auto max-w-3xl space-y-2">
        ...
        <div className="flex items-end gap-1 rounded-md border bg-card p-1.5">
          ...
          <Textarea
            ...
            placeholder={
              isReadOnly ? t`This thread is read-only` : t`Ask anything about this resume`
            }
          ...
          {isStreaming && !isReadOnly ? (
            ...
            <Button
              ...
              disabled={
                isReadOnly || isUploading || (!input.trim() && pendingAttachments.length === 0)
              }
            >
              <PaperPlaneRightIcon />
            </Button>
