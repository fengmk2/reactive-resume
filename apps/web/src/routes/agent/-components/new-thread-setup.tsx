...
export function NewThreadSetup({ resumeId }: NewThreadSetupProps) {
  ...
  const { mutate: createThread, isPending } = useMutation(
    orpc.agent.threads.create.mutationOptions(),
  );
  ...
  const [aiProviderIdOverride, setAiProviderIdOverride] = useState<string | null | undefined>(
    undefined,
  );
  const [sourceResumeIdOverride, setSourceResumeIdOverride] = useState<string | null | undefined>(
    undefined,
  );
      ...
      {providersError ? (
        <div className="rounded-md border border-amber-300 bg-amber-50 p-4 text-amber-950 text-sm dark:bg-amber-950/20 dark:text-amber-200">
          {isAgentConfigError(providersError) ? (
            <Trans>
              AI agent setup is unavailable until REDIS_URL and ENCRYPTION_SECRET are configured.
            </Trans>
          ) : (
          ...
          <div className="relative isolate min-h-32 overflow-hidden rounded-md p-1 lg:p-2">
            ...
            <div className="space-y-3">
              ...
              <Combobox
                ...
                placeholder={
                  isLoadingProviders ? t`Loading providers…` : t`Select a tested provider`
                }
