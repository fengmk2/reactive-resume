...
const textDecorationOptions = [
  ...
  value: NonNullable<StyleIntent["textDecoration"]>;
  label: string;
}[];
const textDecorationStyleOptions = [
  ...
  value: NonNullable<StyleIntent["textDecorationStyle"]>;
  label: string;
}[];
...
const exactFourControlGridClassName =
  "grid grid-cols-1 gap-3 @min-[20rem]:grid-cols-2 @min-[35rem]:grid-cols-4";
...
function CustomStylesSectionForm() {
  ...
  const resetRule = () => {
    updateResumeData((draft) => {
      draft.metadata.styleRules = (draft.metadata.styleRules ?? []).filter(
        (rule) => rule.id !== ruleId,
      );
    });
  };
...
function AppliedRuleCard({
  ...
  return (
    <div
      className={cn(
        "rounded-lg border bg-background/70 p-3 transition-opacity",
...
function RuleScopePill({ target, slot }: RuleScopePillProps) {
  return (
    <div className="inline-flex max-w-full overflow-hidden rounded-sm border border-border/80 bg-background text-xs shadow-xs">
      ...
      <span className="min-w-0 max-w-40 truncate px-2.5 py-1 font-medium text-foreground">
        {slot}
      </span>
    </div>
  );
...
function FontWeightField({ label, id, value, onChange }: FontWeightFieldProps) {
  const options: ComboboxOption<NonNullable<StyleIntent["fontWeight"]>>[] = fontWeightOptions.map(
    (weight) => ({
      value: weight,
      label: weight,
    }),
...
function createMarginSidePatch(
  property: MarginSideProperty,
  value: number | undefined,
): Partial<StyleIntent> {
...
function createTarget({
  ...
  sectionType,
  sectionId,
}: CreateTargetParams): StyleRuleTarget {
...
function getTargetLabel(data: ResumeData, target: StyleRuleTarget) {
  ...
  return (
    getSectionIdOptions(data).find((option) => option.value === target.sectionId)?.label ??
    target.sectionId
  );
}
