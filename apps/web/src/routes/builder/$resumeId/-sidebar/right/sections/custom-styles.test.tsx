...
describe("CustomStylesSectionBuilder", () => {
  ...
  it("renders structured style rule controls", async () => {
    ...
    expect(
      screen.getByLabelText("Text Color").parentElement?.parentElement?.parentElement,
    ).toHaveClass("grid-cols-1", "@min-[20rem]:grid-cols-2", "@min-[35rem]:grid-cols-4");
    expect(
      screen.getByLabelText("Text Color").parentElement?.parentElement?.parentElement,
    ).not.toHaveClass("grid-cols-[repeat(auto-fit,minmax(8rem,1fr))]");
  ...
  it("renames the sidebar entry and uses a distinct icon from design", () => {
    ...
    expect(
      isValidElement(designIcon) &&
        isValidElement(stylesIcon) &&
        designIcon.type !== stylesIcon.type,
    ).toBe(true);
  });
  ...
  it("stores margin and gap intent", () => {
    ...
    const gapRecipe = updateResumeData.mock.calls[1]?.[0] as (draft: {
      metadata: { styleRules: unknown[] };
    }) => void;
  ...
  it("lists applied style rules and toggles individual rules", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Enable All sections: Section container" }),
    ).toBeInTheDocument();
