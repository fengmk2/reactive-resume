...
describe("filterVisibleLayoutSectionIds", () => {
  it("removes item-backed sections with no visible items", () => {
    ...
    expect(filterVisibleLayoutSectionIds(["experience", "volunteer", "references"], data)).toEqual([
      "experience",
    ]);
  });
  ...
  it("keeps layout order for non-empty summary and custom sections", () => {
    ...
    expect(
      filterVisibleLayoutSectionIds(["custom-empty", "summary", "custom-visible"], data),
