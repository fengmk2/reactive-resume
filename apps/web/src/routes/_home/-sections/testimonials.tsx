...
function MarqueeMasonry({ columns, direction, duration = 30 }: MarqueeMasonryProps) {
  ...
  return (
    <m.div
      ...
      animate={{ x: animateX }}
      transition={{
        x: { repeat: Number.POSITIVE_INFINITY, repeatType: "loop", duration, ease: "linear" },
      }}
    >
...
export function Testimonials() {
  const columns = useMemo(() => {
    ...
    for (let index = 0; index < testimonials.length; index += 2) {
      columns.push({
        id: `column-${index / 2}`,
        testimonials: testimonials.slice(index, index + 2),
      });
    }
    ...
    <section id="testimonials" className="overflow-hidden py-12 md:py-16 xl:py-20">
      ...
      >
        ...
        <p className="max-w-4xl text-balance text-muted-foreground leading-relaxed">
          <Trans>
            A lot of people have written to me over the years to share their experiences with
            Reactive Resume and how it has helped them, and I never get tired of reading them. If
            you have a story to share, let me know by sending me an email at{" "}
            <a
              href={`mailto:${email}`}
              target="_blank"
