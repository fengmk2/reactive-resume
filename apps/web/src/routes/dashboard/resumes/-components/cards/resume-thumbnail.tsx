...
function useResumeThumbnail(
  ...
  useEffect(() => {
    if (thumbnailQuery.error)
      console.error("Failed to generate resume thumbnail", thumbnailQuery.error);
...
export function ResumeThumbnail({ isLocked, resume }: ResumeThumbnailProps) {
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "relative size-full overflow-hidden bg-muted/40 transition-all",
        isLocked && "blur-xs",
      )}
    >
