...
describe("rateLimitConfig", () => {
  describe("betterAuth.global", () => {
    ...
    it("rate-limits sign-in/email more strictly than global default", () => {
      expect(rateLimitConfig.betterAuth.global.customRules["/sign-in/email"]).toEqual({
        window: 60,
        max: 5,
      });
    });
    ...
    it("rate-limits sign-up/email even more strictly", () => {
      expect(rateLimitConfig.betterAuth.global.customRules["/sign-up/email"]).toEqual({
        window: 60,
        max: 3,
      });
    });
    ...
    it("rate-limits 2FA verification with longer windows", () => {
      expect(rateLimitConfig.betterAuth.global.customRules["/two-factor/verify-otp"].window).toBe(
        600,
      );
      expect(rateLimitConfig.betterAuth.global.customRules["/two-factor/verify-totp"].window).toBe(
        600,
      );
      expect(
        rateLimitConfig.betterAuth.global.customRules["/two-factor/verify-backup-code"].window,
      ).toBe(600);
    });
