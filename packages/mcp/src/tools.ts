  ...
  return async (params: T): Promise<CallToolResult> => {
    try {
      ...
      return {
        isError: true,
        content: [
          { type: "text", text: `Error ${label}: ${errorMessage(error)}${errorHint(error)}` },
        ],
      };
    }
  };
...
const resumeIdSchema = z
  .string()
  .min(1)
  .describe(`Resume ID. Use \`${T.listResumes}\` to find valid IDs.`);
...
export function registerTools(
  ...
  server.registerTool(
    ...
    withErrorHandling(
      ...
      async ({ tags, sort }: { tags: string[]; sort: "lastUpdatedAt" | "createdAt" | "name" }) => {
        ...
        if (resumes.length === 0)
          return text(`No resumes found. Use \`${T.createResume}\` to create one.`);
  ...
  server.registerTool(
    ...
    withErrorHandling("listing resume tags", async () => {
      ...
      if (tags.length === 0)
        return text("No tags in use yet. Add tags when creating or updating a resume.");
    ...
    T.createResume,
    {
      ...
      inputSchema: z.object({
        name: z
          ...
          .min(1)
          .max(64)
          .describe("Display name for the resume (e.g. 'Software Engineer 2026')"),
        slug: z
          .string()
          .min(1)
          .max(64)
          .describe(
            "URL-friendly slug, must be unique across your resumes (e.g. 'software-engineer-2026')",
          ),
        tags: z
          .array(z.string())
          .optional()
        ...
        withSampleData: z
          ...
          .optional()
          .default(false)
          .describe("Pre-fill with sample data. Default: false."),
      }),
    ...
    T.importResume,
    {
      ...
      inputSchema: z.object({
        data: z
          .unknown()
          .describe(
            "Complete ResumeData JSON (same shape as `read_resume` body or `resume://_meta/schema`).",
          ),
      }),
    ...
    T.duplicateResume,
    {
      ...
      inputSchema: z.object({
        ...
        slug: z
          ...
          .min(1)
          .max(64)
          .describe("URL-friendly slug for the duplicate (must be unique)"),
  ...
  server.registerTool(
    ...
    withErrorHandling(
      "patching resume",
      async ({ id, operations }: { id: string; operations: PatchOperation[] }) => {
    ...
    T.updateResume,
    {
      ...
      inputSchema: z.object({
        ...
        tags: z
          .array(z.string())
          .optional()
          .describe("Replace the resume's tags (omit to leave unchanged)."),
        isPublic: z
          .boolean()
          .optional()
          .describe(
  ...
  server.registerTool(
    ...
    withErrorHandling("locking resume", async ({ id }: { id: string }) => {
      ...
      return text(
        `Resume (${id}) is now locked. It cannot be edited, patched, or deleted until unlocked.`,
      );
    }),
  );
