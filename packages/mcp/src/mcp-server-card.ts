      ...
      inputSchema: toJsonSchemaCompat(
        z.object({
          data: z
            .unknown()
            .describe(
              "Complete ResumeData JSON (same shape as read_resume output or `resume://_meta/schema`).",
            ),
        }),
      ),
  ...
  const prompts = [
    {
      ...
      title: "Build Resume",
      description:
        "Guide the user step-by-step through building a resume from scratch, section by section.",
    ...
    },
    {
      ...
      description:
        "Review resume content and suggest concrete improvements to wording, impact, and structure.",
      arguments: [{ name: "id", description: "Resume ID.", required: true }],
    },
    {
      name: "review_resume",
  ...
  const resourceTemplates = [
    {
      ...
      description:
        "Full resume data as JSON. Discover IDs with the list tool; read via resources/read or read_resume.",
      mimeType: "application/json",
    },
  ];
