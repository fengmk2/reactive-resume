...
describe("websiteSchema", () => {
  it("requires url and label fields", () => {
    expect(websiteSchema.safeParse({ url: "https://example.com", label: "Example" }).success).toBe(
      true,
    );
  });
...
describe("customFieldSchema", () => {
  ...
  it("rejects missing id", () => {
    expect(customFieldSchema.safeParse({ icon: "phone", text: "555-0000", link: "" }).success).toBe(
      false,
    );
  });
