...
describe("shouldUseSectionTimeline", () => {
  it("returns true only for sectionTimeline=true, placement='main', columns=1", () => {
    expect(shouldUseSectionTimeline({ sectionTimeline: true, placement: "main", columns: 1 })).toBe(
      true,
    );
  });
  ...
  it("returns false when placement is not 'main'", () => {
    expect(
      shouldUseSectionTimeline({ sectionTimeline: true, placement: "sidebar", columns: 1 }),
    ).toBe(false);
  });

  it("returns false when columns > 1 (after normalization)", () => {
    expect(shouldUseSectionTimeline({ sectionTimeline: true, placement: "main", columns: 2 })).toBe(
      false,
    );
  });
  ...
  it("returns false when sectionTimeline=false", () => {
    expect(
      shouldUseSectionTimeline({ sectionTimeline: false, placement: "main", columns: 1 }),
    ).toBe(false);
  });

  it("returns true when columns is invalid (normalized to 1)", () => {
    expect(shouldUseSectionTimeline({ sectionTimeline: true, placement: "main", columns: 0 })).toBe(
      true,
    );
  });
});
