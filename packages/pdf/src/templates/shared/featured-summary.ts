...
export const getFeaturedSummaryLayout = ({
  ...
  const featuredSummarySection =
    canFeatureSummary && sections.includes("summary") ? "summary" : undefined;

  return {
    featuredSummarySection,
    regularSections: featuredSummarySection
      ? sections.filter((section) => section !== "summary")
      : sections,
  };
};
