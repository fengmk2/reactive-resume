        ...
        <SectionItemsContext.Provider value={context}>
          <View style={composeStyles(layout.containerStyle, sectionItemsStyle)}>
            {rows.map((row, rowIndex) => (
              ...
              >
                {row}
                {SECTION_ITEM_PLACEHOLDER_KEYS.slice(0, layout.columns - row.length).map(
                  (placeholderKey) => (
...
const SectionItem = ({ children, style }: SectionItemProps) => {
  ...
  if (!useTimeline) {
    return (
      <Div style={composeStyles(itemStyle, itemRuleStyle, sectionItemStyle, style)}>{children}</Div>
    );
  }
  ...
  return (
    <View style={composeStyles(timelineItemStyle)}>
      ...
      <Div style={composeStyles(itemStyle, itemRuleStyle, timelineContentStyle, style)}>
        {children}
      </Div>
    </View>
  );
            ...
            <>
              <View style={composeStyles(splitRowStyle)}>
                ...
                {hasSplitRowText(headerLocation) && (
                  <Text style={composeStyles(alignEndStyle)}>{headerLocation}</Text>
                )}
              </View>
              ...
              {(hasPosition || hasSplitRowText(headerPeriod)) && (
                <View style={composeStyles(splitRowStyle)}>
                  ...
                  {hasSplitRowText(headerPeriod) && (
                    <Text style={composeStyles(alignEndStyle)}>{headerPeriod}</Text>
                  )}
                </View>
              )}
            </>
    ...
    <SectionShell sectionId={sectionId} title={education.title}>
      <SectionItems columns={education.columns}>
        {items.map((item) => {
          ...
          const { top: headerDegreeAndGrade, bottom: headerLocationAndPeriod } =
            promoteSplitRowRight({
              top: degreeAndGrade,
...
const CustomSection = ({ sectionId, showHeading = true }: CustomSectionProps) => {
  ...
  return match(customSection.type)
    ...
    .with("cover-letter", () => (
      <CoverLetterSection section={customSection as CustomItemSection<CoverLetterItem>} />
    ))
    .exhaustive();
};
