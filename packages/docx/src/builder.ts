...
function buildHeader(data: ResumeData, colorHex: string, textColorHex: string): Paragraph[] {
  ...
  const addSeparator = () => {
    if (contactParts.length > 0) {
      contactParts.push(
        new TextRun({ text: "  |  ", color: "999999", size: bodySize, font: bodyFont }),
      );
    }
  };
  ...
  if (basics.phone) {
    addSeparator();
    contactParts.push(
      new TextRun({ text: basics.phone, size: bodySize, font: bodyFont, color: textColorHex }),
    );
  }
  ...
  if (basics.location) {
    addSeparator();
    contactParts.push(
      new TextRun({ text: basics.location, size: bodySize, font: bodyFont, color: textColorHex }),
    );
  }
  ...
  for (const field of basics.customFields) {
    ...
    if (field.link) {
      ...
      if (customLink) {
        ...
        contactParts.push(
          new TextRun({ text: field.text, size: bodySize, font: bodyFont, color: textColorHex }),
        );
      }
    } else {
      contactParts.push(
