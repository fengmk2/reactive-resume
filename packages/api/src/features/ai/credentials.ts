...
export function decryptCredential(payload: string) {
  ...
  return Buffer.concat([decipher.update(decode(encodedCiphertext)), decipher.final()]).toString(
    "utf8",
  );
}

export function redactEncryptedCredential(
  fields: StoredCredentialFields,
