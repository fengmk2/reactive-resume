...
describe("AI provider base URL policy", () => {
  ...
  it("blocks private and non-HTTPS provider URLs by default", () => {
    ...
    expect(() =>
      resolveAiBaseUrl({ provider: "openai-compatible", baseURL: "https://localhost:11434/v1" }),
    ).toThrow("INVALID_AI_BASE_URL");
    expect(() =>
      resolveAiBaseUrl({ provider: "openai-compatible", baseURL: "http://example.com/v1" }),
    ).toThrow("INVALID_AI_BASE_URL");
  });
  ...
  it("allows private and non-HTTPS provider URLs when explicitly enabled", () => {
    ...
    expect(
      resolveAiBaseUrl({ provider: "openai-compatible", baseURL: "http://localhost:11434/v1" }),
    ).toBe("http://localhost:11434/v1");
    expect(
      resolveAiBaseUrl({ provider: "openai-compatible", baseURL: "https://10.0.0.5/v1" }),
  ...
  it("rejects non-HTTP schemes even when unsafe provider URLs are enabled", () => {
    ...
    expect(() =>
      resolveAiBaseUrl({ provider: "openai-compatible", baseURL: "file:///etc/passwd" }),
    ).toThrow("INVALID_AI_BASE_URL");
    expect(() =>
      resolveAiBaseUrl({ provider: "openai-compatible", baseURL: "ftp://example.com/v1" }),
    ).toThrow("INVALID_AI_BASE_URL");
  });
});
