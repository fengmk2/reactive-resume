...
function parseAndValidateResumeJson(resultText: string): ResumeData {
  ...
  if (diagnostics.coercions.length === 0 && diagnostics.droppedSectionItems.length === 0)
    return data;

  const droppedBySection = diagnostics.droppedSectionItems.reduce<Record<string, number>>(
    (acc, item) => {
...
export function getModel(input: GetModelInput) {
  ...
  return match(provider)
    ...
    .with("openrouter", () =>
      createOpenAICompatible({ name: "openrouter", apiKey, baseURL }).languageModel(model),
    )
...
export function getAgentModel(input: GetModelInput) {
  ...
  return createOpenAI({ apiKey: input.apiKey, baseURL: resolveAiBaseUrl(input) }).responses(
    input.model,
  );
}
...
export async function testConnection(input: TestConnectionInput): Promise<boolean> {
  ...
  const result = await generateText({
    ...
    messages: [
      { role: "user", content: `Respond only with JSON Object: { "result": "${RESPONSE_OK}" }` },
    ],
  });
