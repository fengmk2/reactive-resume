...
function orderByLastUsedAtDescNullsLast() {
  return desc(
    sql<Date>`coalesce(${schema.aiProvider.lastUsedAt}, '1970-01-01T00:00:00.000Z'::timestamptz)`,
  );
}
  ...
  getRunnableById: async (input: { id: string; userId: string }) => {
    ...
    if (!provider.enabled || provider.testStatus !== "success") {
      throw new ORPCError("BAD_REQUEST", {
        message: "AI provider must be tested and enabled before use.",
      });
    }
  ...
  update: async (input: UpdateAiProviderInput) => {
    ...
    const nextBaseUrl =
      input.baseURL !== undefined
        ? normalizeBaseUrl({ provider, baseURL: input.baseURL })
        : existing.baseUrl;
    ...
    if (input.enabled === true && existing.testStatus !== "success" && !runtimeChanged) {
      throw new ORPCError("BAD_REQUEST", {
        message: "AI provider must be tested successfully before enabling.",
      });
    }
    ...
    const [updated] = await db
      ...
      .set({
        ...
        ...(runtimeChanged
          ? { enabled: false, testStatus: "untested", lastTestedAt: null, testError: null }
          : {}),
        ...encrypted,
      })
      .where(and(eq(schema.aiProvider.id, input.id), eq(schema.aiProvider.userId, input.userId)))
