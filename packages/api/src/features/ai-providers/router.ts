...
export const aiProvidersRouter = {
  list: protectedProcedure
    .route({
      ...
      description:
        "Lists saved provider/model/API key combinations for the authenticated user. API keys are redacted.",
    })
  ...
  create: protectedProcedure
    .route({
      ...
      description:
        "Stores an encrypted provider/model/API key combination. The key is never returned.",
    })
    .input(providerInput)
    .output(type<AiProviderResponse>())
    .errors({
  ...
  test: protectedProcedure
    .route({
      ...
      description:
        "Decrypts the saved API key server-side and validates the provider/model connection.",
    })
    .input(z.object({ id: z.string() }))
    .output(type<AiProviderResponse>())
