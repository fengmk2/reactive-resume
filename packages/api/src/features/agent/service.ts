...
const ROLLED_BACK_MESSAGE =
  "This patch was rolled back when the resume was restored to an earlier state.";
...
function withAttachmentUiParts(
  message: UIMessage,
...
function withoutAgentAttachmentUiParts(message: UIMessage): UIMessage {
  return {
    ...
    parts: message.parts.filter(
      (part) => !(part.type === "file" && part.url.startsWith(AGENT_ATTACHMENT_URL_PREFIX)),
    ),
  };
}
...
): part is AnsweredAskUserQuestionPart {
...
function mergeAskUserQuestionOutputs(
...
export function buildAttachmentModelParts(
...
async function getUnlinkedMessageAttachments(input: {
  ids: unknown;
  threadId: string;
  userId: string;
}) {
...
async function readAttachmentModelInputs(
  attachments: AgentAttachmentRecord[],
): Promise<AttachmentModelInput[]> {
...
async function persistMessage(input: {
  ...
  await db
    ...
    .where(
      and(eq(schema.agentThread.id, input.threadId), eq(schema.agentThread.userId, input.userId)),
    );

  return message;
}

async function updateAssistantToolResultMessage(input: {
  userId: string;
  threadId: string;
  message: UIMessage;
}) {
  ...
  const existingRow = existingRows.find(
    (row) => row.role === "assistant" && toMessage(row).id === input.message.id,
  );
  if (!existingRow) {
    throw new ORPCError("BAD_REQUEST", {
...
export const agentService = {
  threads: {
    list: async (input: { userId: string }) => {
      ...
      const rows = await db
        ...
        .where(
          and(eq(schema.agentThread.userId, input.userId), isNull(schema.agentThread.deletedAt)),
        )
    ...
    get: async (input: { id: string; userId: string }) => {
      ...
      const [messages, actions, attachments, resume] = await Promise.all([
        ...
        db
          ...
          .where(
            and(
              eq(schema.agentAction.threadId, input.id),
        ...
        db
          ...
          .where(
            and(
              eq(schema.agentAttachment.threadId, input.id),
              eq(schema.agentAttachment.userId, input.userId),
            ),
          )
        ...
        thread.workingResumeId
          ? resumeService
              .getById({ id: thread.workingResumeId, userId: input.userId })
              .catch(() => null)
          : null,
      ]);
    ...
    archive: async (input: { id: string; userId: string }) => {
      ...
      await db
        ...
        .where(
          and(eq(schema.agentThread.id, input.id), eq(schema.agentThread.userId, input.userId)),
        );
    },
  ...
  messages: {
    send: async (input: SendMessageInput) => {
      ...
      try {
        ...
        const attachmentModelParts = buildAttachmentModelParts(
          await readAttachmentModelInputs(attachmentsForModel),
        );
      ...
      try {
        ...
        if (activeRunId) {
          ...
          try {
            ...
            if (persistError)
              console.error(
                "[agent] Failed to clear active run after stop persistence error",
                error,
              );
          }
        }
  ...
  attachments: {
    create: async (input: CreateAttachmentInput) => {
      ...
      if ((attachmentCount?.total ?? 0) >= MAX_ATTACHMENTS_PER_MESSAGE)
        throw new ORPCError("BAD_REQUEST");
  ...
  actions: {
    revert: async (input: { id: string; userId: string }) => {
      ...
      const [action] = await db
        ...
        .from(schema.agentAction)
        .where(
          and(eq(schema.agentAction.id, input.id), eq(schema.agentAction.userId, input.userId)),
        )
        .limit(1);
      ...
      if (action.kind !== "resume_patch") {
        throw new ORPCError("BAD_REQUEST", {
          message: "Only resume patch actions can be rolled back.",
        });
      }
      const resumeId = action.resumeId;
      const snapshotData = action.snapshotData;
      if (!resumeId)
        throw new ORPCError("BAD_REQUEST", { message: "The edited resume no longer exists." });
      if (!snapshotData) {
        throw new ORPCError("BAD_REQUEST", {
          message: "This legacy patch does not have a rollback snapshot.",
        });
      }
