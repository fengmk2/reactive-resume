...
describe("agent tools", () => {
  ...
  it("keeps instructions explicit about native search availability", () => {
    ...
    expect(buildAgentInstructions({ hasProviderNativeSearch: true })).toContain(
      "user-provided public URLs",
    );
    expect(buildAgentInstructions({ hasProviderNativeSearch: false })).not.toContain(
      "Use web_search",
    );
