  ...
  return {
    ...
    metadata: {
      ...
      page: {
        margin: 14,
        format: "a4" as const,
        options: { breakLine: false, pageNumbers: false },
      },
...
describe("ReactiveResumeV4JSONImporter – custom section description-only items", () => {
  ...
  it("treats whitespace-only name the same as missing name", () => {
    ...
    const result = importer.parse(JSON.stringify(v4));
    const item = result.customSections[0]?.items[0] as {
      company: string;
      position: string;
      description: string;
    };
...
describe("ReactiveResumeV4JSONImporter – skill level scaling (v4: 0-10 → v5: 0-5)", () => {
  function makeWithSkills(
    items: Array<{ id: string; visible: boolean; name: string; level: number }>,
  ) {
    return makeV4Base({
      skills: {
        name: "Skills",
