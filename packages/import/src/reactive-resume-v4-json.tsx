...
const convertFontVariantToWeight = (
  variant: string,
  defaultWeight: FontWeight = "400",
): FontWeight => FONT_WEIGHT_MAP[variant.toLowerCase()] ?? defaultWeight;

const convertFontVariants = (
    ...
    try {
      ...
      const transformed: ResumeData = {
        picture: {
          ...
          borderColor: v4Data.basics.picture?.effects?.border
            ? "rgba(0, 0, 0, 0.5)"
            : "rgba(0, 0, 0, 0)",
        ...
        customSections: Object.entries(v4Data.sections.custom ?? {}).map(
          ([sectionId, section]) => ({
        ...
        metadata: {
          ...
          design: {
            colors: {
              primary: v4Data.metadata.theme?.primary
                ...
                : "rgba(220, 38, 38, 1)",
              text: v4Data.metadata.theme?.text
                ? colorToRgba(v4Data.metadata.theme.text)
                : "rgba(0, 0, 0, 1)",
          ...
          typography: {
            ...
            heading: {
              fontFamily: v4Data.metadata.typography?.font?.family ?? "IBM Plex Serif",
              fontWeights: convertFontVariantsForHeading(
                v4Data.metadata.typography?.font?.variants,
              ),
              fontSize: clampFontSize(
                convertAndClampFontSize(v4Data.metadata.typography?.font?.size ?? 14.67) + 3,
              ),
              lineHeight: clampLineHeight(v4Data.metadata.typography?.lineHeight ?? 1.5),
            },
          },
