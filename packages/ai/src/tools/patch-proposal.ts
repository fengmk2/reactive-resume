...
function getSectionLabel(segments: string[]): string {
  if (segments[0] === "sections" && segments[1])
    return fieldLabels[segments[1]] ?? titleize(segments[1]);
  if (segments[0] === "metadata" && segments[1])
    return `${sectionLabels.metadata} ${titleize(segments[1])}`;
...
function getPreviewAfterValue(patchedData: ResumeData, operation: JsonPatchOperation): unknown {
  ...
  if (operation.op === "add" && decodeJsonPointerPath(operation.path).at(-1) === "-")
