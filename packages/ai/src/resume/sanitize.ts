...
function normalizeResumeDataForSchema(
  ...
  for (const sectionKey of Object.keys(sectionRequiredFieldMap) as SectionKey[]) {
    ...
    const itemTemplate = aiExtractionTemplate.sections[sectionKey].items[0] as Record<
      string,
      unknown
    >;
...
export function sanitizeAndParseResumeJson(resultText: string): ResumeSanitizationResult {
  ...
  try {
    ...
    const normalizedData = normalizeResumeDataForSchema(
      coercedData as Record<string, unknown>,
      diagnostics,
    );
