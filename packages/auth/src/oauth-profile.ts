...
async function normalizeExistingUserEmail(
  userId: string,
  currentEmail: string,
...
async function findAvailableUsernameSuffix(
  baseUsername: string,
  index = 1,
): Promise<string | null> {
...
export function createProfileMapper<TProfile extends OAuthProfile>({
  ...
  return async (profile: TProfile) => {
    ...
    const existingUser =
      (await findExistingUser?.(profile, context)) ?? (await findExistingUserByEmail(email));
