...
const isRateLimitEnabled =
  process.env.NODE_ENV === "production" && !env.FLAG_DISABLE_API_RATE_LIMIT;
  ...
  if (isCustomOAuthProviderEnabled()) {
    authConfigs.push({
      ...
      mapProfileToUser: createProfileMapper({
        ...
        getPreferredUsername: (profile, context) =>
          profile.preferred_username ?? context.emailLocalPart,
        getName: (profile, context) =>
          profile.name ?? profile.preferred_username ?? context.emailLocalPart,
