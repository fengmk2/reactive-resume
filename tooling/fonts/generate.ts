...
const argLimit = args.includes("--limit")
...
function variantToWeight(variant: Variant): Weight | null {
  if (["100", "200", "300", "500", "600", "700", "800", "900"].includes(variant))
    return variant as Weight;
...
export async function generateFonts() {
  ...
  const googleFontResults: WebFont[] = filteredItems.slice(0, argLimit).map((item) => {
    ...
    const weights: Weight[] = item.variants
  ...
  const jsonString = argCompress
    ? JSON.stringify(allWebFonts)
    : JSON.stringify(allWebFonts, null, 2);
  await mkdir(FONTS_DIR, { recursive: true });
  await writeFile(WEBFONTLIST_FILE, jsonString, "utf-8");

  console.log(
    `Generated ${allWebFonts.length} fonts in the list (including Computer Modern web fonts).`,
  );
}
